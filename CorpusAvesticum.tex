\ProvidesFile{CorpusAvesticum.tex}[2020/05/30 v0.34.0 Configuration file for
brill]
...
\newcommand{\@stanzafiles}{}
\newcommand{\Importstanzas}[1]{%
   \@onlypreamble
   \IfFileExists{#1}%
      {\AtBeginDocument{\directlua{read_stanzas_from_file("\luaescapestring{\unexpanded{#1}}")}}}%
      {\ClassError{brill}{File `#1` could not be found!}{}}%
}

\AfterEndPreamble{%
   \InputIfFileExists{./stanzas.tex}%
      {\ClassInfo{brill}{Stanza macros successfully loaded from `stanzas.tex`!}}%
      {\ClassInfo{brill}{There is no file `stanzas.tex`}}%
}
...
%       Please use \string\SetBibliographyFile\space}}%
   ...
   \ifdefempty{\@stanzafiles}%
      {\ClassInfo{brill}{Could not find !}}%      
      {\@stanzafiles%
         \ClassInfo{brill}{Successfully loaded }%
      }%
}%
