\ProvidesFile{CorpusAvesticum.tex}[2020/08/01 v0.51.0 Configuration file for
brill]
...
\newcommand{\indexheaderfont}[1]{{\LARGE\bfseries #1}}
...
\newcommand{\@newlanguage}[2]{%
   \expandafter\newcommand\csname #1\endcsname[2][]{%
      ...
      \begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}##2\end{nohyphens}%
...
\newcommand{\@@newlanguage}[2]{%
   \expandafter\newcommand\csname #1\endcsname[2][]{%
      ...
      \begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}\emph{##2}\end{nohyphens}%
      \ifstrempty{##1}{}{\ \enquote{##1}}%
      \resetlanguage} 
   \newenvironment{#1Block}{%
      \def\indexlanguage{#2}%
...
\newcommand*{\@Av}[2][]{%
   ...
   \renewcommand*{\idxentryfont}[1]{\textit{##1}}%
   \Word[#2]{\begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}\emph{#2}\end{nohyphens}}%
   \ifstrempty{#1}{}{\ \enquote{#1}}%
   \resetlanguage}
\newcommand*{\@@Av}[2][]{%
   \def\indexlanguage{Avestan}%
...
\newcommand*{\@OP}[3][]{%
   ...
   \renewcommand*{\idxentryfont}[1]{\textit{##1}}%
   \begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}\textit{#2}\end{nohyphens}%
   \ifstrempty{#3}{}{\ [#3]}%
   \ifstrempty{#1}{}{\ \enquote{#1}}%
   \resetlanguage
...
\newcommand*{\@Phlreverse}[3][]{%
   ...
   \ifstrempty{#3}
      ...
      {%
         \textlangle{}#3\textrangle{}%
         \ifstrempty{#2}{}{\ \begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}\textit{(#2)}\end{nohyphens}}%   
      }%
   % We assume, that #2 and #3 are not both empty
   \ifstrempty{#1}{}{\ \enquote{#1}}%
   \resetlanguage%
...
\newcommand*{\Phltlt}[2][]{%
   \def\indexlanguage{Pahlavi}%
   \renewcommand*{\idxentryfont}[1]{##1}%
   \ifstrempty{#2}{}{\begin{nohyphens}\addfontfeatures{SmallCapsFeatures={RawFeature={-smcp;-c2sc}}}#2\end{nohyphens}}%
...
\newenvironment{AvBlock}{%
   \def\indexlanguage{Avestan}%
   \renewcommand*{\idxentryfont}[1]{\textit{##1}}\itshape\hyphenpenalty=10000\exhyphenpenalty=10000\relax}%
   {\resetlanguage}
\let\AvstBlock\AvBlock
\let\endAvstBlock\endAvBlock
\newenvironment{BactrBlock}{%
   \def\indexlanguage{Bactrian}%
   \renewcommand*{\idxentryfont}[1]{##1}\hyphenpenalty=10000\exhyphenpenalty=10000\relax}%
   {\resetlanguage}
\let\FaBlock\NPBlock
...
\newcommand{\app}[1]{%
   ...
   \unskip\realsuperscript{\theappcrfootnote}%
   \@firstrdgtrue%
   \gappto{\@appcr}{\item }#1%
}
...
\newenvironment{stanza}[1]{%
   ...
   \iftoggle{@stanzabooktoindex}%
      {%
         %\long\def\@temph{\minisec}%
         %\Ifstr{\@stanzaheading}{\@temph}%
         %   {\minisec{\Av{\passage{#1}}}}%
...
\def\GetstanzaTextonly{\@ifstar{\@editedtextonlytrue\@GetstanzaTextonly}{\@editedtextonlyfalse\@GetstanzaTextonly}}
   ...
   \Ifstr{#1}{(date, dedication of day and month)}%
      ...
      {%
         ...
         \else
            ...
            {\itshape\StrSubstitute{#1}{ }{\realsuperscript{\upshape\thenextappcrfootnote} }}%
         \fi%
      }%
}
\newcommand{\editedtranslationtext}[1]{\begin{nohyphens}\editedtext{#1}\end{nohyphens}}
\newcommand{\editedcommentarytext}[1]{\begin{nohyphens}\editedtext{#1}\end{nohyphens}}
% We need a macro for text portions within the Avestan (italic) text.
% In the XML these are <foreign> elements
\newcommand{\foreign}[2][]{%
   \IfStrEqCase{#1}{%
      {pal-Phlv}{\Phl{#2}}%
      {fa}{\MP{#2}}%TODO: Correct language?
%      {}{}
   }[\textup{#2}]%
}
%%%
...
\newcommand{\@gettransnewline}{%
   \immediate\write\@auxout{%
      \csgdef{gettrans\@currvalgettrans}{}}%
   \@linebreakwithindent%
}
...
\newcommand{\@Gettranslation}[1]{%
   \begingroup%
      ...
      \let\@normalcr\@linebreakwithindent%
      \let\RDTrans\@gobble%
      \par\noindent%
      \csuse{translation-#1}%
      \@linebreakwithindent%
...
\newcommand{\@@Gettranslation}[2][]{%
   \refstepcounter{gettrans}%
   \xdef\@currvalgettrans{\thegettrans}%
   \begingroup%
      \let\newline\@gettransnewline%
      \let\\\@gettransnewline%
      \setlength{\parindent}{0pt}%
      \par\noindent%
      \IfStrEqCase{#1}{%
      ...
      }[\ifcsdef{gettrans\@currvalgettrans}%
