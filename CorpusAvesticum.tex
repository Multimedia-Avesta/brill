\ProvidesFile{CorpusAvesticum.tex}[2019/08/21 v0.12.1 Configuration file for 
brill]
...
\begin{luacode*}
   function modifySorting()
      ...
      f:seek('set')
      -- f:close()
      content = string.gsub(content, "(Y~?)(%d+)%.(%d+)",
      function(a,b,c) return
         string.format("Y%03d.%03d@%s%d.%d", b, c, a, b, c) end)
      -- content = string.gsub(content, "(Y~?)(%d+)%.(%u)", 
      -- function(a,b,c) return
      --   string.format("Y%03d.%s@%s%d.%s", b, c, a, b, c) end)         
      -- content = string.gsub(content, "(Par~?)(%d+)%.(%d+)", 
      -- function(a,b,c) return
      --   string.format("Par%03d.%03d@%s%d.%d", b, c, a, b, c) end)
      -- os.execute("timeout " .. tonumber(10))         
      -- local f = io.open("passages.idx", "w")
      f:write(content)
      f:close()
   end
\end{luacode*}
...
%\ExplSyntaxOn
%
%\NewDocumentCommand{\makesortentry}{m}{%
%   \tl_set:Nn \sortentry { #1 }
%   \regex_replace_all:nnN { (\d+)\.(\d+) } { 
%   \\three@digits{\0}-\\three@digits{\1} } \sortentry
%}
%
%\ExplSyntaxOff
%\newcommand{\@makesortentry}[1]{
%   \splitatdot{#1}\@leftsort\@rightsort
%   \edef\@sortentry{%
%      \IfInteger{\@leftsort}
%         {\three@digits{\@leftsort}}
%         {\@leftsort}%
%      .%
%      \IfInteger{\@rightsort}
%         {\three@digits{\@rightsort}}%
%         {\@rightsort}%
%      }%
%}  
%\newcommand{\splitatdot}[3]{%
%  \protected@edef\split@temp{#1}%
%  \expandafter\split@dot\split@temp.\@nil{#2}{#3}%
%}
%\def\split@dot#1.#2\@nil#3#4{%
%  \def#3{#1}%
%  \if\relax\detokenize{#2}\relax
%    % no dot
%    \let#4\@empty
%  \else
%    \split@eatdot#2\@nil{#4}%
%  \fi
%}
%\def\split@eatdot#1.\@nil#2{\def#2{#1}}
...
\newcommand*{\Ank}[1]{Ank~#1}% Afrin i Gahambar
